...
You can export all of a user's data by using the "export" rake task. The user's name must be passed in parameter.
ex : @bundle exec rake 'export[username]'@

The generated tgz file will contain three files :
* export.opml : The OPML export of all feeds and folders of the user.
* favorites.html : The HTML export of all favorites of the user.
* items.json : All feed items in JSON, grouped by feed.
